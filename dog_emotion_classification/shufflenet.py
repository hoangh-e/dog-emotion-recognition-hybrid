...
def load_shufflenet_model(model_path, architecture='shufflenet_v2_x1_0', num_classes=4, input_size=224, device='cuda'):
    ...
    # Create ShuffleNet model based on architecture
    valid_architectures = ['shufflenet_v2_x0_5', 'shufflenet_v2_x1_0', 'shufflenet_v2_x1_5', 'shufflenet_v2_x2_0']
    
    if architecture.lower() not in valid_architectures:
        raise ValueError(f"Unsupported ShuffleNet architecture: {architecture}. "
    ...
    # Create preprocessing transform for ShuffleNet
...
def get_shufflenet_transforms(input_size=224, is_training=True):
    """
    Get preprocessing transforms for ShuffleNet model.
...
def predict_emotion_shufflenet_v2_x0_5(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using ShuffleNet V2 x0.5."""
    return predict_emotion_shufflenet(image_path, model, transform, head_bbox, device)


def predict_emotion_shufflenet_v2_x1_0(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using ShuffleNet V2 x1.0."""
    return predict_emotion_shufflenet(image_path, model, transform, head_bbox, device)


def predict_emotion_shufflenet_v2_x1_5(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using ShuffleNet V2 x1.5."""
    return predict_emotion_shufflenet(image_path, model, transform, head_bbox, device)


def predict_emotion_shufflenet_v2_x2_0(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using ShuffleNet V2 x2.0."""
    return predict_emotion_shufflenet(image_path, model, transform, head_bbox, device) 