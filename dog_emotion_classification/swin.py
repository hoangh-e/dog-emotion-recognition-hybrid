...
def load_swin_model(model_path, architecture='swin_t', num_classes=4, input_size=224, device='cuda'):
    ...
    # Create Swin Transformer model based on architecture
    valid_architectures = ['swin_t', 'swin_s', 'swin_b', 'swin_v2_t', 'swin_v2_s', 'swin_v2_b']
    
    if architecture.lower() not in valid_architectures:
        raise ValueError(f"Unsupported Swin architecture: {architecture}. "
    ...
    # Create preprocessing transform for Swin Transformer
...
def get_swin_transforms(input_size=224, is_training=True):
    """
    Get preprocessing transforms for Swin Transformer model.
...
def predict_emotion_swin_t(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using Swin Transformer Tiny."""
    return predict_emotion_swin(image_path, model, transform, head_bbox, device)


def predict_emotion_swin_s(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using Swin Transformer Small."""
    return predict_emotion_swin(image_path, model, transform, head_bbox, device)


def predict_emotion_swin_b(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using Swin Transformer Base."""
    return predict_emotion_swin(image_path, model, transform, head_bbox, device)


def predict_emotion_swin_v2_t(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using Swin Transformer V2 Tiny."""
    return predict_emotion_swin(image_path, model, transform, head_bbox, device)


def predict_emotion_swin_v2_s(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using Swin Transformer V2 Small."""
    return predict_emotion_swin(image_path, model, transform, head_bbox, device)


def predict_emotion_swin_v2_b(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using Swin Transformer V2 Base."""
    return predict_emotion_swin(image_path, model, transform, head_bbox, device) 