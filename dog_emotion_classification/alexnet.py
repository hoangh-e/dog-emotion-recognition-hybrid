...
def load_alexnet_model(model_path, num_classes=4, input_size=224, device='cuda'):
    ...
    print(f"🔄 Loading AlexNet model from: {model_path}")
    
    # Create AlexNet model
    model = models.alexnet(pretrained=False)
    print(f"🏗️  Created AlexNet base model")
    ...
    # Create preprocessing transform for AlexNet
...
def predict_emotion_alexnet(image_path, model, transform, head_bbox=None, device='cuda',
    ...
    try:
        ...
        print(f"❌ Error in AlexNet emotion prediction: {e}")
        raise RuntimeError(f"AlexNet prediction failed: {e}")


def get_alexnet_transforms(input_size=224, is_training=True):
    """
    Get preprocessing transforms for AlexNet model.
...
def predict_emotion_alexnet_simple(image_path, model, transform, head_bbox=None, device='cuda'):
    """Predict emotion using AlexNet (simplified interface)."""
    return predict_emotion_alexnet(image_path, model, transform, head_bbox, device) 